...
const Navbar = () => {
    ...
    const menuItems = [
        ...
        { title: 'Testimonials', href: '#testimonials', sectionId: 'testimonials' },
        { title: 'Blog', href: '#blog', sectionId: 'blog' },
        { title: 'Contact', href: '#contact', sectionId: 'contact' },
        { title: 'About', href: '/about', isUnique: true }
    ];
    ...
    const handleScroll = useCallback(throttle(() => {
        ...
        if (location.pathname === '/') {
            ...
            if (sectionsInView.length > 0) {
                ...
                const matchingItem = menuItems.find(item => item.sectionId === currentSectionId);
    ...
    useEffect(() => {
        ...
        const currentItem = menuItems.find(item => {
            ...
            if (currentPath === '/about' && item.href === '/about') return true;
            return false;
        });
            ...
            <div className="container mx-auto px-4">
                <div className="flex justify-between items-center h-16">
                    ...
                    <div className="hidden md:flex items-center space-x-1">
                        <div className={`p-1 rounded-full ${scrolled ? 'glass-card' : 'bg-dark-900/30'}`}>
                                    ...
                                    <Link
                                        ...
                                        className={`relative px-4 py-2 text-sm font-medium rounded-full transition-all duration-300 ${
                                            activeItem === item.title 
                                                ? 'text-white' 
                                                : scrolled ? 'text-dark-300 hover:text-white' : 'text-white/70 hover:text-white'
                        ...
                        <div className="pl-2">
                            <Link 
                                ...
                                className={`neo-button group inline-block ${
                                    activeItem === 'Contact' ? 'active-contact' : ''
                                }`}
                            >
                                <span className="absolute inset-0 border border-white/20 rounded-full"></span>
                                <span className="relative z-10 text-white text-sm">Contact Us</span>
                                <span className={`neo-button-effect bg-gradient-to-r from-secondary-500 to-accent-500 ${
                                    activeItem === 'Contact' ? 'opacity-100' : ''
                                }`}></span>
                            </Link>
                        </div>
                    </div>
                    ...
                    >
                        <div className={`absolute inset-0 rounded-full ${isOpen ? 'bg-accent-500/20' : 'bg-transparent'} transition-all duration-300`}></div>
                        {isOpen ? (
                            <FaTimes className={`relative z-10 ${scrolled ? 'text-white' : 'text-white'}`} />
                        ) : (
                            <FaBars className={`relative z-10 ${scrolled ? 'text-white' : 'text-white'}`} />
                        )}
                    </button>
                </div>
                ...
                >
                    <div className="glass-dark backdrop-blur-lg mt-2 rounded-b-xl p-4 mx-4 space-y-3 border-t border-white/5">
                        ...
                        <div className="pt-2">
                            <Link 
                                to="/#contact"
                                className={`w-full neo-button group inline-block ${
                                    activeItem === 'Contact' ? 'active-contact' : ''
                                }`}
                                onClick={() => {
                                    if (location.pathname === '/') {
